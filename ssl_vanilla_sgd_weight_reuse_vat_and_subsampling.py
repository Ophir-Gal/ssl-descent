# -*- coding: utf-8 -*-
"""SSL_vanilla_SGD_weight_reuse_VAT_and_subsampling.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1PgM4L21XG4SgMEjG87FE-KZkJZMs-DPi
...
def get_semi_sup_train_test_loaders(percent_unlabeled=0):
  datadir = 'data'
  set_seed(0)
  device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
  ...
  # Here, make the unlabelled data set. 
  # Set certain number of items in y_train as -1. percent_unlabeled% of samples are unlabelled
  labels_to_drop = np.random.choice(len(y_train),
                  size=int(len(y_train) * (percent_unlabeled/100.0)),
                  replace=False)
...
def get_model_and_optim(n_inputs=MNIST_DEFAULT_NUM_INPUTS, n_hidden=2, n_classes=10, lr=0.0001,
  ...
  optimizer = optim.SGD(vat.parameters(), lr=lr)
  print(summary(model, (model.input_size,)))
...
"""# CODE FOR SSL EXPERIMENTS"""

def run_exp(percent_unlabeled=20):
  train_loader, test_loader = get_semi_sup_train_test_loaders(percent_unlabeled)
  ...
  plt.xlabel('No. parameters (×10^3)')
  plt.legend(loc="upper right")
  plt.savefig('ssl_vat_{}%_unlab.png'.format(percent_unlabeled), dpi=300)
  plt.show()

  # plot entire history (log scaled)
  ...
  plt.savefig('ssl_vat_{}%_unlab_log_scaled.png'.format(percent_unlabeled), dpi=300)
  plt.show()

"""# Exp. 1 (20% unlabeled)"""

run_exp(percent_unlabeled=20)

"""# Exp. 2 (40% unlabeled)"""

run_exp(percent_unlabeled=40)

"""# Exp. 3 (60% unlabeled)"""

run_exp(percent_unlabeled=60)

"""# Exp. 4 (80% unlabeled)"""

run_exp(percent_unlabeled=80)